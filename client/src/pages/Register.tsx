...
const Register: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validateEmail(email)) {
      setError(t('registrationFailed') || 'Invalid email format');
      return;
    }
    if (!validatePassword(password)) {
      setError(t('registrationFailed') || 'Password must be at least 6 characters');
      return;
    }
    setLoading(true);
    setError('');
  ...
  return (
    ...
    >
      <div className="max-w-md w-full bg-gray-800 p-8 rounded-lg">
        ...
        <div className="mt-6">
          <h3 className="text-center mb-4">{t('orRegisterWith')}</h3>
          <div className="space-y-2">
            {i18n.language === 'zh' ? (
              <>
                <button onClick={() => handleOAuth('wechat')} className="w-full p-2 bg-green-500 rounded">{t('social.wechat')}</button>
                <button onClick={() => handleOAuth('qq')} className="w-full p-2 bg-blue-500 rounded">QQ</button>
              </>
            ) : i18n.language === 'ja' ? (
              <>
        ...
        <p className="text-center mt-4">
          {t('hasAccount')} <Link to="/login" className="text-indigo-400">{t('login')}</Link>
        </p>
      </div>
    </motion.div>
  );
